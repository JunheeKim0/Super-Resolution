...
def get_hr_lr(start, end, folder_names, path, data_shape, scaling_factor, transform):
    ...
    with alive_bar(len(folder_names[start:end]), force_tty=True) as bar:
        for idx in range(start, end):
            sub_path = "{}{}".format('/storage/kjh/diffusion/DWI', folder_names[idx])
...
def create_npy_dataset(path, modality, output_path, data_shape, scaling_factor=4, transform=None):
    ...
    training_hr, training_lr = get_hr_lr(0, 5662, folder_names, path, data_shape, scaling_factor, transform)

    with open(output_path + f'/HCP_{modality}_scale_by_{scaling_factor}_imgs.npy', 'wb') as f:
