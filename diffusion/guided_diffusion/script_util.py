import sys
sys.path.append('D:/JH/DisC-Diff')
...
from guided_diffusion.unet import SuperResModel
...
def get_ssim(ground_truth, generated_image):
    ...
    min_size = min(ground_truth.shape[0], ground_truth.shape[1])

    # win_size는 최소 측면 길이보다 작은 홀수여야 함
    win_size = min_size if min_size % 2 == 1 else min_size - 1

    # win_size가 7 미만인 경우, SSIM 계산 불가
    if win_size < 7:
        raise ValueError("Image too small for default win_size of 7. Consider resizing.")

    return structural_similarity(ground_truth, generated_image, win_size=win_size, multichannel=True, gaussian_weights=True, data_range=data_range, channel_axis = -1)
...
def sr_create_model(
    ...
    return SuperResModel(
        ...
        out_channels=(in_channel if not learn_sigma else in_channel * 2),
